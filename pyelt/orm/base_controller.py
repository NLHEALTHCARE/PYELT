...
class Controller:
    ...
    def new(self, bk='', runid=0) -> DvEntity:
        ...
        entity.__dict__['type'] = self.entity_cls.get_name()
    ...
    def load_old(self, bk='', filter='') -> List[DvEntity]:
        ...
        if not filter:
            filter = '1 = 1'
        view_name = self.__class__.entity_cls.get_view_name()
    ...
    def load_by_hub(self, bk='', filter='') -> List[DvEntity]:
        ...
        if not filter:
            filter = '1 = 1'
        hub = self.__class__.entity_cls.get_hub_name()
    ...
    def load_sat(self, sat):
        print(sat.get_name())
        params = {'dv': 'dv', 'sat': sat.__class__.get_name(), 'id': sat._id}
    ...
    def save_new_hub(self, entity, source_system='sys'):
        ...
        params['hub'] = entity.__class__.get_hub_name()
...
WHERE NOT EXISTS (SELECT 1 FROM {dv}.{hub} WHERE bk='{bk}') RETURNING _id;""".format(**params)
    ...
    def save_sat(self, sat, source_system='sys'):
        params = {}
        params['dv'] = self.dwh.dv.name
        params['sat'] = sat.__class__.get_name()
        field_names = ''
        field_values = ''
