        ...
        try:
            ...
            if isinstance(mappings.source, dict):
                ...
                insert_sql = """
                    ...
                    INSERT INTO {dv}._ref_values (_runid, _active, _source_system, _insert_date, _revision, valueset_oid, valueset_naam, code, weergave_naam, niveau, niveau_type, temp_id, temp_fk)
                    SELECT DISTINCT {runid}, True, '{source_system}', now(), 0, {source_oid_field}, {source_type_field}, {source_code_field}, {source_descr_field}, {source_level_field}, {source_leveltype_field}, {source_tempid_field}, {source_tempfk_field}
                    FROM {sor}.{sor_table} hstg
                    WHERE floor(hstg._runid) = floor({runid})
                      AND hstg._valid AND hstg._active
                ...
                insert_sql = """
                    ...
                    INSERT INTO {dv}._ref_values (_runid, _active, _source_system, _insert_date, _revision, valueset_naam, code, weergave_naam, niveau, niveau_type, temp_id, temp_fk)
                    SELECT DISTINCT {runid}, True, '{source_system}', now(), 0, '{ref_type}', {source_code_field}, NULL, NULL, NULL, NULL, NULL
                    FROM {sor}.{sor_table} hstg
                    WHERE floor(hstg._runid) = floor({runid})
                      AND hstg._valid AND hstg._active
                      AND NOT EXISTS (SELECT 1 FROM {dv}._ref_values ref WHERE ref.valueset_naam = '{ref_type}' AND ref.code = hstg.{source_code_field}
                                    AND ref.weergave_naam = hstg.{source_descr_field} AND ref.niveau = {source_level_field} AND ref.niveau_type = {source_leveltype_field) AND ref.temp_id = {source_tempid_field} AND ref.temp_fk = {source_tempfk_field};""".format(**params)
            else:
                insert_sql = """
                    INSERT INTO {dv}._ref_valuesets (_runid, _source_system, _insert_date, naam, oid)
                    SELECT {runid}, '{source_system}', now(), '{ref_type}', NULL
                    ...
                    INSERT INTO {dv}._ref_values (_runid, _active, _source_system, _insert_date, _revision, valueset_naam, code, weergave_naam, niveau, niveau_type, temp_id, temp_fk))
                    SELECT DISTINCT {runid}, True, '{source_system}', now(), 0, '{ref_type}', {source_code_field}, {source_descr_field}, {source_level_field}, {source_leveltype_field}, {source_tempid_field}, {source_tempfk_field}
                    FROM {sor}.{sor_table} hstg
                    WHERE floor(hstg._runid) = floor({runid})
                      AND hstg._valid AND hstg._active
                      AND NOT EXISTS (SELECT 1 FROM {dv}._ref_values ref WHERE ref.valueset_naam = '{ref_type}' AND ref.code = hstg.{source_code_field}
                                    AND ref.weergave_naam = {source_descr_field} AND ref.niveau = {source_level_field} AND ref.niveau_type = {source_leveltype_field) AND ref.temp_id = {source_tempid_field} AND ref.temp_fk = {source_tempfk_field};""".format(**params)
